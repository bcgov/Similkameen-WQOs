...
# This script is to continue assessing censored data, take out high non-detects, make a summary table
# of the censored data analysis, detemine which method to use for censoring
...
# Load HC data
HC_sites <- read.csv("data/report/HC_Plots/tables/HC_sites.csv", stringsAsFactors = FALSE, header=TRUE)

#remove all values <0 (negative values throw an error in ROS)
HC_sites <- HC_sites %>% filter(Value>0)

#remove unnescessary columns
HC_sites <- dplyr::select(HC_sites, -c(Code, ResultLetter, Station)) 

# Summary Stats for table 1 - based upon uncensored data 

table.1 <- HC_sites %>% group_by(EMS_ID, Variable) %>%
...
cens_data <- filter(HC_sites, CENSOR == TRUE)
...
write.csv(table.1,'data/report/HC_plots/tables/Table1_censored.csv', row.names = FALSE)

# Determine non-detects by station to select method.
# Count samples that are D vs ND
method_selection <- HC_sites %>%
...
# Join to data set
HC_sites %<>% left_join(select(method_selection, EMS_ID, Variable,
                               METHOD))
...
write.csv(method_summary,'data/report/HC_plots/tables/MethodSummary.csv', row.names = FALSE)
...
g1_data <- HC_sites %>% filter(METHOD == "0.5 MDL") %>%
  group_by(EMS_ID, Variable) %>%
...
# arithmetic mean
g2_data <- HC_sites %>% filter(METHOD == "0.5 MDL - MEAN") %>%
...
g3_data <- HC_sites %>% filter(METHOD == "ROS")
...
g4_data <- HC_sites %>% filter(METHOD == "MEAN") %>%
  group_by(EMS_ID, Variable) %>%
...
write.csv(summary_stats,'data/report/HC_plots/tables/SummaryStats.csv', row.names = FALSE)
...
sumstats <- read.csv("data/report/HC_plots/tables/SummaryStats.csv", 
...
for (i in 1:length(parameters)){
  ...
  ggsave(filename=paste0("C:/R Projects/Similkameen-WQOs/data/report/HC_Plots/",x,".tiff"), units="in", width=9, height=6, dpi=300, compression = 'lzw')
