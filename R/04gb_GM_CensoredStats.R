...
# Load GM data
GM_Sites <- read.csv("data/report/GM_Plots/tables/GM_Sites.csv", 
                     stringsAsFactors = FALSE, header=TRUE)

#remove all values <0 (negative values throw an error in ROS)
GM_Sites <- GM_Sites %>% filter(Value>0)

#remove unnescessary columns
GM_Sites <- dplyr::select(GM_Sites, -c(Code, ResultLetter, Station)) 

# Summary Stats for table 1 - based upon uncensored data 

table.1 <- GM_Sites %>% group_by(EMS_ID, Variable) %>%
...
cens_data <- filter(GM_Sites, CENSOR == TRUE)
...
write.csv(table.1,'data/report/GM_Plots/tables/Table1_censored.csv', row.names = FALSE)

# Determine non-detects by station to select method.
# Count samples that are D vs ND
method_selection <- GM_Sites %>%
...
# Join to data set
GM_Sites %<>% left_join(select(method_selection, EMS_ID, Variable,
                               METHOD))
...
write.csv(method_summary,'data/report/GM_Plots/tables/MethodSummary.csv', row.names = FALSE)
...
g1_data <- GM_Sites %>% filter(METHOD == "0.5 MDL") %>%
  group_by(EMS_ID, Variable) %>%
...
# arithmetic mean
g2_data <- GM_Sites %>% filter(METHOD == "0.5 MDL - MEAN") %>%
...
g3_data <- GM_Sites %>% filter(METHOD == "ROS")
...
g4_data <- GM_Sites %>% filter(METHOD == "MEAN") %>%
  group_by(EMS_ID, Variable) %>%
...
write.csv(summary_stats,'data/report/GM_Plots/tables/SummaryStats.csv', row.names = FALSE)
...
sumstats <- read.csv("data/report/GM_plots/tables/SummaryStats.csv", 
                     stringsAsFactors = FALSE, header=TRUE)
...
for (i in 1:length(parameters)){
  ...
  ggsave(filename=paste0("C:/R Projects/Similkameen-WQOs/data/report/GM_Plots/",x,".tiff"), units="in", width=9, height=6, dpi=300, compression = 'lzw')
