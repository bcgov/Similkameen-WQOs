...
# Load SC data
SC_sites <- read.csv("data/report/SC_Plots/tables/SC_sites.csv", 
                     stringsAsFactors = FALSE, header=TRUE)

#remove all values <0 (negative values throw an error in ROS)
SC_sites <- SC_sites %>% filter(Value>0)

#remove unnescessary columns
SC_sites <- dplyr::select(SC_sites, -c(Code, ResultLetter, Station)) 

# Summary Stats for table 1 - based upon uncensored data 

table.1 <- SC_sites %>% group_by(EMS_ID, Variable) %>%
...
cens_data <- filter(SC_sites, CENSOR == TRUE)
...
write.csv(table.1,'data/report/SC_Plots/tables/Table1_censored.csv', row.names = FALSE)

# Determine non-detects by station to select method.
# Count samples that are D vs ND
method_selection <- SC_sites %>%
...
# Join to data set
SC_sites %<>% left_join(select(method_selection, EMS_ID, Variable,
                               METHOD))
...
write.csv(method_summary,'data/report/SC_Plots/tables/MethodSummary.csv', row.names = FALSE)
...
g1_data <- SC_sites %>% filter(METHOD == "0.5 MDL") %>%
  group_by(EMS_ID, Variable) %>%
...
# arithmetic mean
g2_data <- SC_sites %>% filter(METHOD == "0.5 MDL - MEAN") %>%
...
g3_data <- SC_sites %>% filter(METHOD == "ROS")
...
g4_data <- SC_sites %>% filter(METHOD == "MEAN") %>%
  group_by(EMS_ID, Variable) %>%
...
write.csv(summary_stats,'data/report/SC_Plots/tables/SummaryStats.csv', row.names = FALSE)
...
sumstats <- read.csv("data/report/SC_plots/tables/SummaryStats.csv", 
                     stringsAsFactors = FALSE, header=TRUE)
...
for (i in 1:length(parameters)){
  ...
  ggsave(filename=paste0("C:/R Projects/Similkameen-WQOs/data/report/SC_Plots/",x,".tiff"), units="in", width=9, height=6, dpi=300, compression = 'lzw')
