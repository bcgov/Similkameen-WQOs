...
# Load KR data
KR_sites <- read.csv("data/report/OC_CC_Plots/tables/OC_CC_sites.csv", 
                     stringsAsFactors = FALSE, header=TRUE)


#remove unnescessary columns
KR_sites <- dplyr::select(KR_sites, -c(Code, ResultLetter, Station)) 

#remove all values <0 (negative values throw an error in ROS)
KR_sites <- KR_sites %>% filter(Value>0)

# Summary Stats for table 1 - based upon uncensored data 

table.1 <- KR_sites %>% group_by(EMS_ID, Variable) %>%
...
cens_data <- filter(KR_sites, CENSOR == TRUE)
...
write.csv(table.1,'data/report/OC_CC_Plots/tables/Table1_censored.csv', row.names = FALSE)

# Determine non-detects by station to select method.
# Count samples that are D vs ND
method_selection <- KR_sites %>%
...
# Join to data set
KR_sites %<>% left_join(select(method_selection, EMS_ID, Variable,
                               METHOD))
...
write.csv(method_summary,'data/report/OC_CC_Plots/tables/MethodSummary.csv', row.names = FALSE)
...
g1_data <- KR_sites %>% filter(METHOD == "0.5 MDL") %>%
  group_by(EMS_ID, Variable) %>%
...
# arithmetic mean
g2_data <- KR_sites %>% filter(METHOD == "0.5 MDL - MEAN") %>%
...
g3_data <- KR_sites %>% filter(METHOD == "ROS")
...
g4_data <- KR_sites %>% filter(METHOD == "MEAN") %>%
  group_by(EMS_ID, Variable) %>%
...
write.csv(summary_stats,'data/report/OC_CC_Plots/tables/SummaryStats.csv', row.names = FALSE)
...
sumstats <- read.csv("data/report/OC_CC_plots/tables/SummaryStats.csv", 
                     stringsAsFactors = FALSE, header=TRUE)


## First set working directory to save plots to. 
setwd('C:/R Projects/Similkameen-WQOs/data/report/OC_CC_Plots/WQOs')
...
for (i in 1:length(parameters)){
  ...
  ggsave(filename=paste0("C:/R Projects/Similkameen-WQOs/data/report/OC_CC_Plots/",x,".tiff"), units="in", width=9, height=6, dpi=300, compression = 'lzw')
