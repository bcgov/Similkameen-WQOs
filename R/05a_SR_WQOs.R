...
plotdata <- read_csv("data/report/SR_plots/tables/SR_sites2.csv")
...
plotdata$EMS_ID <- factor(plotdata$EMS_ID, 
                          levels=c( "0500075", "BC08NL0001", "0500724", "0500725", 
                                    "E207463", "E317970", "E317971", "E318570",
                                    "E317973", "BC08NL0005"))

units <- distinct(plotdata, Units)

# set working drive for plots
setwd('C:/R Projects/Similkameen-WQOs/data/report/SR_Plots/WQOs')
...
(tsslimits<-TSS_WQO(plotdata, "0500075", 10, 10, 100))

#write csv of TSS threshold, data shows clear flows for threshold is 10 mg/L 
write.csv(tsslimits,'C:/R Projects/Similkameen-WQOs/data/report/SR_Plots/tables/SR_tss.csv', row.names = FALSE)
...
## plot tss against threshold for 05000075
...
tssplot2 <- ggplot(tsslimits2, aes(x = DateTime, y = Value, col = Variable, shape=CENSOR)) +
  ...
  labs(col = "0500075", shape="<MDL")
plot(tssplot2)
ggsave(filename = "TSS_WQO 0500075.png", plot = tssplot2, h=4, w=6, units="in", dpi=300)
...
#Total Phosphorus
#remove <MDL=3 at 0500075
plotdata <- plotdata %>% 
  dplyr::filter(!(EMS_ID == "0500075" & Variable == "phosphorus total" & Value==0.3))
...
#remove <MDL=6 at E207463
plotdata <- plotdata %>% dplyr::filter(!((DateTime=="2000-05-16"|
       DateTime == "2000-05-25") & Variable == "chromium total" & Value==6))
...
crplot <- ggplot(site_cr, aes(x = DateTime, y = Value, color = EMS_ID, shape = CENSOR)) +
  geom_point(size=0.75) + 
  geom_hline(aes(yintercept = 20,linetype = "WQO Acute"), color = "blue") +
  geom_hline(aes(yintercept = 2,linetype = "WQO Chronic"), color = "red") +
  scale_linetype_manual(name = "T Cr", values = c(2, 2),  
...
crplot <- ggplot(site_cr, aes(x = DateTime, y = Value, color = EMS_ID, shape = CENSOR)) +
  geom_point(size=0.75) + 
  geom_hline(aes(yintercept = 2,linetype = "WQO Chronic"), color = "blue") +
  geom_hline(aes(yintercept = 1,linetype = "WWQG Cr(VI) AL"), color = "red") +
  scale_linetype_manual(name = "T Cr", values = c(2, 2),  
                        guide = guide_legend(override.aes = list(color = c("blue", "red")))) +
  theme_bw() +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  ...
  theme(text=element_text(size=8)) +
  xlab("Date") +
  ylab("Chromium"~~(mu*g/L)) + coord_cartesian(ylim=c(0,7.5)) +
  theme(text=element_text(size=8)) +
  labs(color="Station", shape="<MDL")+
  facet_wrap(~Variable, ncol=2, labeller = as_labeller(plotnames))
plot(crplot)
...
Crplot <- ggplot(site_Cr, aes(x = DateTime, y = Value, shape = CENSOR)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Total Chromium"~~(mu*g/L)) + coord_cartesian(ylim=c(0,12)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  labs(shape="<MDL")+
  ...
  facet_wrap(~EMS_ID, ncol=2, drop=FALSE)
plot(Crplot)
ggsave(filename = "TCr_facet2.png", plot = Crplot, h=4, w=6, units="in", dpi=300)
...
asplot <- ggplot(site_as, aes(x = DateTime, y = Value, color = EMS_ID, shape = CENSOR)) +
  ...
  scale_linetype_manual(name = "T As", values = c(2, 2),  
...
feplot <- ggplot(site_fe, aes(x = DateTime, y = Value, color = EMS_ID, shape = CENSOR)) +
  geom_point(size=0.75) + 
  geom_hline(aes(yintercept = 0.35,linetype = "WQG Acute AL D Fe"), color = "blue") +
  geom_hline(aes(yintercept = 0.3,linetype = "WQO=WQG DW T Fe"), color = "red") +
...
feplot <- ggplot(site_fe, aes(x = DateTime, y = Value, shape = CENSOR)) +
  ...
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  labs(shape="<MDL")+  coord_cartesian(ylim= c(0,15)) +
  theme(strip.text = element_text(size=4, margin = margin(.2, 0, .2, 0, "mm"))) +
  facet_wrap(~EMS_ID, ncol=2, drop=FALSE)
plot(feplot)
...
feplot <- ggplot(site_fe, aes(x = DateTime, y = Value, shape = CENSOR)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Dissolved Iron (mg/L)") +  coord_cartesian(ylim= c(0,1)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  labs(shape="<MDL")+
...
moplot <- ggplot(site_mo, aes(x = DateTime, y = Value, color = EMS_ID)) +
  ...
  scale_linetype_manual(name = "WQO/WQG T Mo", values = c(1, 1),  
                        guide = guide_legend(override.aes = list(color = "red"))) +
  theme_bw() +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
...
urplot <- ggplot(site_ur, aes(x = DateTime, y = Value, color = EMS_ID)) +
  geom_point(size=0.75) + 
  geom_hline(aes(yintercept = 10,linetype = "WQO=WWQG Chronic IR"), color = "blue") +
...
##remove very high values over 50 mg/L
plotdata <- plotdata %>% dplyr::filter(!((EMS_ID=="BC08NL0005" & 
                                            Value >= 50)))
                                         
site_do<- filter(plotdata, Variable %in% "oxygen dissolved")
doplot <- ggplot(site_do, aes(x = DateTime, y = Value, color = EMS_ID)) +
  geom_point(size=0.75) + 
  geom_hline(aes(yintercept = 11,linetype = "April-June"), color = "red") +
  geom_hline(aes(yintercept = 8,linetype = "July-March"), color = "red") +
  scale_linetype_manual(name = "WQO", values = c(2, 2),  
                        guide = guide_legend(override.aes = list(color = c("red", "red")))) +
  theme_classic() +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
...
# filter out data to compare to WQO, April to June
site_do<- filter(plotdata, Variable %in% "oxygen dissolved")

AprJun <- site_do[month(site_do$DateTime) >= 4 & month(site_do$DateTime) <= 6,]

doplot <- ggplot(AprJun, aes(x = DateTime, y = Value, color = EMS_ID)) +
  geom_point(size=0.75) + 
  geom_hline(aes(yintercept = 11,linetype = "April-June"), color = "red") +
  ...
  labs(color="Station")
plot(doplot)
ggsave(filename = "DO_AprJun.png", plot = doplot, h=4, w=6, units="in", dpi=300)

# filter out data to compare to WQO, July - March
JulDec <- site_do[month(site_do$DateTime) >= 7 & month(site_do$DateTime) <= 12,]
JanMar <- site_do[month(site_do$DateTime) >= 1 & month(site_do$DateTime) <= 3,]
#rejoin dates
JulMar <- dplyr::union(JulDec, JanMar)

doplot <- ggplot(JulMar, aes(x = DateTime, y = Value, color = EMS_ID)) +
  geom_point(size=0.75) + 
  geom_hline(aes(yintercept = 8,linetype = "July-March"), color = "red") +
  scale_linetype_manual(name = "WQO", values = c(1, 1),  
                        guide = guide_legend(override.aes = list(color = "red"))) +
  ...
  labs(color="Station")
plot(doplot)
ggsave(filename = "DO_JulMar.png", plot = doplot, h=4, w=6, units="in", dpi=300)
...
alplot <- ggplot(site_al, aes(x = DateTime, y = Value, color = EMS_ID, shape = CENSOR)) +
  ...
  scale_linetype_manual(name = "WQO/WQG D-Al", values = c(2, 2),  
                        guide = guide_legend(override.aes = list(color = c("blue", "red")))) +
  theme_bw() +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
...
alplot <- ggplot(site_al, aes(x = DateTime, y = Value, color = EMS_ID, shape = CENSOR)) +
  ...
  theme(text=element_text(size=8)) +
  xlab("Date") +
  ylab("Aluminum (mg/L)") + coord_cartesian(ylim=c(0,7)) +
  theme(text=element_text(size=8)) +
  labs(color="Station", shape = "<MDL")+
  facet_wrap(~Variable, ncol=2, labeller = as_labeller(plotnames))
plot(alplot)
ggsave(filename = "T+D Al2.png", plot = alplot, h=4, w=6, units="in", dpi=300)
...
alplot <- ggplot(site_al, aes(x = DateTime, y = Value, shape = CENSOR)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Total Aluminum (mg/L)") + coord_cartesian(ylim=c(0,15)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  labs(shape="<MDL")+
...
alplot <- ggplot(site_al, aes(x = DateTime, y = Value, shape = CENSOR)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Dissolved Aluminum (mg/L)") + coord_cartesian(ylim=c(0,1)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  labs(shape="<MDL")+
...
mnplot <- ggplot(site_mn, aes(x = DateTime, y = Value, color = EMS_ID)) +
  geom_point(size=0.75) + 
  geom_hline(aes(yintercept = 0.05,linetype = "Acute"), color = "red") +
  scale_linetype_manual(name = "WQ0", values = c(1, 1),  
                        guide = guide_legend(override.aes = list(color = "red"))) +
  theme_bw() +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  xlab("Date") +
  ylab("Managanese (mg/L)") + coord_cartesian(ylim=c(0,0.6)) +
theme(text=element_text(size=8)) +
  labs(color="Station")+
  facet_wrap(~Variable, ncol=2, labeller = as_labeller(plotnames))
plot(mnplot)
...
mnplot <- ggplot(site_mn, aes(x = DateTime, y = Value)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Dissolved Manganese (mg/L)") + coord_cartesian(ylim=c(0,0.6)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  theme(strip.text = element_text(size=4, margin = margin(.2, 0, .2, 0, "mm"))) +
...
mnplot2 <- ggplot(site_mn3, aes(x = DateTime, y = Value, col = Variable)) +
  ...
  labs(col = "0500075")
plot(mnplot2)
ggsave(filename = "T+D Mn0500075.png", plot = mnplot2, h=4, w=6, units="in", dpi=300)

#T&D Mn BC08NL0001 w/WQGs
...
mnplot2 <- ggplot(site_mn4, aes(x = DateTime, y = Value, col = Variable)) +
  ...
  labs(col = "BC08NL0001")
plot(mnplot2)
ggsave(filename = "T+D MnBC08NL0001.png", plot = mnplot2, h=4, w=6, units="in", dpi=300)

#T&D Mn BC08NL0005 w/WQGs
...
mnplot2 <- ggplot(site_mn5, aes(x = DateTime, y = Value, col = Variable)) +
  ...
  labs(col = "BC08NL0005")
plot(mnplot2)
ggsave(filename = "T+D MnBC08NL0005.png", plot = mnplot2, h=4, w=6, units="in", dpi=300)
...
pbplot <- ggplot(site_pb, aes(x = DateTime, y = Value, shape = CENSOR)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Total Lead"~~(mu*g/L)) + coord_cartesian(ylim=c(0,10)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  theme(strip.text = element_text(size=4, margin = margin(.2, 0, .2, 0, "mm"))) +
...
pbplot <- ggplot(site_pb, aes(x = DateTime, y = Value, shape = CENSOR)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Dissolved Lead"~~(mu*g/L)) + coord_cartesian(ylim=c(0,0.6)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  theme(strip.text = element_text(size=4, margin = margin(.2, 0, .2, 0, "mm"))) +
...
pbplot2 <- ggplot(site_pb3, aes(x = DateTime, y = Value, col = Variable, shape=CENSOR)) +
  ...
  labs(col = "0500075", shape= "<MDL")
plot(pbplot2)
ggsave(filename = "T+D pb0500075.png", plot = pbplot2, h=4, w=6, units="in", dpi=300)

#T&D Pb BC08NL0001 w/WQGs
site_pb4 <- filter(site_pb2, Variable %in% c("lead total", 
                                            "lead dissolved",
                                            "AcuteAL", "ChronicAL") 
                   & EMS_ID == "BC08NL0001")
pbplot2 <- ggplot(site_pb4, aes(x = DateTime, y = Value, col = Variable, shape=CENSOR)) +
  geom_point(size=0.75) + 
  theme_bw() +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  ...
  xlab("Date") +
  ylab("Concentration"~~(mu*g/L)) + coord_cartesian(ylim=c(0,130)) +
  labs(col = "BC08NL0001", shape="<MDL)")
plot(pbplot2)
ggsave(filename = "T+D PbBC08NL0001.png", plot = pbplot2, h=4, w=6, units="in", dpi=300)

#T&D Pb BC08NL0005 w/WQGs
site_pb5 <- filter(site_pb2, Variable %in% c("lead total", 
                                            "lead dissolved",
                                            "AcuteAL", "ChronicAL")  
                   & EMS_ID == "BC08NL0005")
pbplot2 <- ggplot(site_pb5, aes(x = DateTime, y = Value, col = Variable, shape=CENSOR)) +
  geom_point(size=0.75) + 
  theme_bw() +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  ...
  xlab("Date") +
  ylab("Concentration"~~(mu*g/L)) + coord_cartesian(ylim=c(0,130)) +
  labs(col = "BC08NL0005", shape="<MDL")
plot(pbplot2)
ggsave(filename = "T+D PbBC08NL0005.png", plot = pbplot2, h=4, w=6, units="in", dpi=300)
...
cdplot2 <- ggplot(site_cd2, aes(x = DateTime, y = Value, col = Variable, shape=CENSOR)) +
  ...
  labs(col = "0500075", shape="<MDL")
plot(cdplot2)
ggsave(filename = "T+D cd0500075.png", plot = cdplot2, h=4, w=6, units="in", dpi=300)

#T&D Cd BC08NL0001 w/WQGs
site_cd2 <- filter(site_cd, Variable %in% c("cadmium dissolved",
                                            "AcuteAL", "ChronicAL") 
                   & EMS_ID == "BC08NL0001")
#plot data in specific order
site_cd2$Variable <- factor(site_cd2$Variable, 
...
cdplot2 <- ggplot(site_cd2, aes(x = DateTime, y = Value, col = Variable, shape=CENSOR)) +
  ...
  labs(col = "BC08NL0001", shape="<MDL")
plot(cdplot2)
ggsave(filename = "T+D CdBC08NL0001.png", plot = cdplot2, h=4, w=6, units="in", dpi=300)

#T&D Cd BC08NL0005 w/WQGs
site_cd2 <- filter(site_cd, Variable %in% c("cadmium dissolved",
                                            "AcuteAL", "ChronicAL")  
                   & EMS_ID == "BC08NL0005")
#plot data in specific order
...
cdplot2 <- ggplot(site_cd2, aes(x = DateTime, y = Value, col = Variable, shape=CENSOR)) +
  ...
  labs(col = "BC08NL0005", shape="<MDL")
plot(cdplot2)
ggsave(filename = "T+D CdBC08NL0005.png", plot = cdplot2, h=4, w=6, units="in", dpi=300)
...
znplot <- ggplot(site_zn, aes(x = DateTime, y = Value, color = EMS_ID, shape=CENSOR)) +
  geom_point(size=0.75) + 
  geom_hline(aes(yintercept = 30,linetype = "Acute AL"), color = "blue") +
  geom_hline(aes(yintercept = 10,linetype = "Chronic AL"), color = "red") +
  scale_linetype_manual(name = "WQO T-Zn", values = c(2, 2),  
                        guide = guide_legend(override.aes = list(color = c("blue", "red")))) +
  theme_bw() +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
...
so4plot <- ggplot(site_so4, aes(x = DateTime, y = Value, shape = CENSOR)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Dissolved Sulfate (mg/L)") + coord_cartesian(ylim=c(0,45)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  theme(strip.text = element_text(size=4, margin = margin(.2, 0, .2, 0, "mm"))) +
...
noplot <- ggplot(site_no, aes(x = DateTime, y = Value, color = EMS_ID, shape=CENSOR)) +
  ...
  scale_linetype_manual(name = "WQO T-NO2-", values = c(2, 2),  
                        guide = guide_legend(override.aes = list(color = c("red", "blue")))) +
  theme_bw() +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
...
noplot <- ggplot(site_no, aes(x = DateTime, y = Value, color=EMS_ID, shape = CENSOR)) +
  ...
  labs(shape="<MDL")
plot(noplot)
ggsave(filename = "D NO3_2.png", plot = noplot, h=4, w=6, units="in", dpi=300)
...
noplot <- ggplot(site_no, aes(x = DateTime, y = Value, color=EMS_ID, shape = CENSOR)) +
  ...
  theme_classic() +
  xlab("Date") +
  ylab("Dissolved Nitrate (mg/L)") + coord_cartesian(ylim=c(0,0.17)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  theme(strip.text = element_text(size=4, margin = margin(.2, 0, .2, 0, "mm"))) +
  labs(shape="<MDL")
plot(noplot)
ggsave(filename = "D NO3_3.png", plot = noplot, h=4, w=6, units="in", dpi=300)
...
spcplot <- ggplot(site_spc, aes(x = DateTime, y = Value)) +
  ...
  facet_wrap(~EMS_ID, ncol=2, drop=FALSE)
plot(spcplot)
ggsave(filename = "Cond-field facet.png", plot = spcplot, h=4, w=6, units="in", dpi=300)
...
docplot <- ggplot(site_doc, aes(x = DateTime, y = Value, color = EMS_ID, shape=CENSOR)) +
  geom_point(size=0.75) + 
  theme_bw() +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
...
ntuplot <- ggplot(site_ntu, aes(x = DateTime, y = Value)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Turbidity (NTU)") + coord_cartesian(ylim=c(0,300)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(strip.text = element_text(face="bold", size=4, margin = margin(.2, 0, .2, 0, "mm"))) +
  theme(text=element_text(size=8)) +
...
ntu_WQO <- turbidity_WQO(plotdata, "0500075", 1, 5, 10, 5, 50)

#write csv of turbidity threshold, data shows clear flows for threshold is 10 mg/L 
write.csv(ntu_WQO,'C:/R Projects/Similkameen-WQOs/data/report/SR_Plots/tables/SR_turbidity.csv', row.names = FALSE)
...
## plot turbidity against threshold for 05000075
...
ntuplot2 <- ggplot(ntu_WQO, aes(x = DateTime, y = Value, col = Variable)) +
  ...
  labs(col = "0500075", shape="<MDL")
plot(ntuplot2)
ggsave(filename = "Turbidity_WQO 0500075.png", plot = ntuplot2, h=4, w=6, units="in", dpi=300)
...
#only data for 0500075, E207463, and both fed/prov stations
NH4_SR <- filter(NH4WQGs, EMS_ID == "0500075" | EMS_ID == "BC08NL0001" | EMS_ID ==  "E207463" | 
                   EMS_ID == "BC08NL0005")


#plot data in specific order
NH4_SR$EMS_ID <- factor(NH4_SR$EMS_ID, 
                            levels=c("0500075", "BC08NL0001", "E207463", "BC08NL0005" )) 
                                     

NH4plot2 <- ggplot(NH4_SR, aes(x = DateTime, y = Value, color = Variable, shape = CENSOR)) +
  geom_point(size=0.75) + 
  theme_bw() +
  xlab("Date") + 
  ylab("Dissolved Ammonia (mg/L)") + 
...
## make dataframe of Similkameen River sites
#only data for 0500075, E317970, and both fed/prov stations
Cu_SR <- filter(CuWQGs, EMS_ID == "0500075" | EMS_ID == "BC08NL0001" |  EMS_ID == "E317970" | 
                 EMS_ID == "BC08NL0005")

#plot data in specific order
Cu_SR$EMS_ID <- factor(Cu_SR$EMS_ID, 
                        levels=c("0500075", "BC08NL0001", "E317970", "BC08NL0005" )) 

#Plot cu with chronic guideline
site_Cu2 <- filter(Cu_SR, Variable %in% c("copperdissolved", "Chronic.WQG"))
...
site_Cu3 <- filter(Cu_SR, Variable %in% c("copperdissolved", "Acute.WQG"))
...
#filter data from 2012 for closer look at recent
Cu_SR2 <- filter(Cu_SR, DateTime > "2014-01-01" & DateTime < "2019-12-31")

#Plot cu with chronic guideline
site_Cu2 <- filter(Cu_SR2, Variable %in% c("copperdissolved", "Chronic.WQG"))
Cuplot2 <- ggplot(site_Cu2, aes(x = DateTime, y = Value, color = Variable, shape = CENSOR)) +
  geom_point(size=0.75) + 
  theme_bw() +
  xlab("Date") +
  ...
  facet_wrap(~EMS_ID, ncol=2)
plot(Cuplot2)
ggsave(filename = "D Cu_Chronic_facet2.png", plot = Cuplot2, h=4, w=6, units="in", dpi=300)

#Plot Cu with Acute WQG
site_Cu3 <- filter(Cu_SR2, Variable %in% c("copperdissolved", "Acute.WQG"))
Cuplot3 <- ggplot(site_Cu3, aes(x = DateTime, y = Value, color = Variable, shape = CENSOR)) +
  geom_point(size=0.75) + 
  theme_bw() +
  xlab("Date") +
  ...
  facet_wrap(~EMS_ID, ncol=2)
plot(Cuplot3)
ggsave(filename = "D Cu_Acute_facet2.png", plot = Cuplot3, h=4, w=6, units="in", dpi=300)
