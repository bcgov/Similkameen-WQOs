...
# Load WC data
WC_sites <- read.csv("data/report/Tribs/tables/tribs.csv", 
                     stringsAsFactors = FALSE, header=TRUE)

#remove all values <0 (negative values throw an error in ROS)
WC_sites <- WC_sites %>% filter(Value>0)

#remove unnescessary columns
WC_sites <- dplyr::select(WC_sites, -c(Code, ResultLetter, Station)) 

# Summary Stats for table 1 - based upon uncensored data 

table.1 <- WC_sites %>% group_by(EMS_ID, Variable) %>%
...
cens_data <- filter(WC_sites, CENSOR == TRUE)
...
write.csv(table.1,'data/report/Tribs/tables/Table1_censored.csv', row.names = FALSE)

# Determine non-detects by station to select method.
# Count samples that are D vs ND
method_selection <- WC_sites %>%
...
# Join to data set
WC_sites %<>% left_join(select(method_selection, EMS_ID, Variable,
                               METHOD))
...
write.csv(method_summary,'data/report/Tribs/tables/MethodSummary.csv', row.names = FALSE)
...
g1_data <- WC_sites %>% filter(METHOD == "0.5 MDL") %>%
  group_by(EMS_ID, Variable) %>%
...
# arithmetic mean
g2_data <- WC_sites %>% filter(METHOD == "0.5 MDL - MEAN") %>%
...
g3_data <- WC_sites %>% filter(METHOD == "ROS")
...
g4_data <- WC_sites %>% filter(METHOD == "MEAN") %>%
  group_by(EMS_ID, Variable) %>%
...
write.csv(summary_stats,'data/report/Tribs/tables/SummaryStats.csv', row.names = FALSE)
...
sumstats <- read.csv("data/report/Tribs/tables/SummaryStats.csv", 
                     stringsAsFactors = FALSE, header=TRUE)

## order sites from upstream to downstream
sumstats$EMS_ID <- factor(sumstats$EMS_ID, 
                          levels=c( "0500083", "E318572", "E206637", "E318571"))
                               

## First set working directory to save plots to. 
setwd('C:/R Projects/Similkameen-WQOs/data/report/Tribs/WQOs')
...
for (i in 1:length(parameters)){
  ...
  ggsave(filename=paste0("C:/R Projects/Similkameen-WQOs/data/report/Tribs/",x,".tiff"), units="in", width=9, height=6, dpi=300, compression = 'lzw')
