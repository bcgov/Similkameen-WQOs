...
parm <- distinct(SC_sites, Variable)

## make dataframe of Keremeos River sites
SC_sites <- filter(sim_data, EMS_ID == "E215954" | EMS_ID == "E250751" |
                     EMS_ID == "E206634")

## filter out the dates to plot, only including data from 2000 on

SC_sites <- filter(SC_sites, DateTime > "2000-01-01" & DateTime < "2019-12-31")

# Set censor for non-detect
SC_sites %<>% mutate(CENSOR = ifelse(is.na(ResultLetter) | ResultLetter == "M",
                                     FALSE, TRUE))
...
write.csv(SC_sites,'C:/R Projects/Similkameen-WQOs/data/report/SC_Plots/tables/SC_sites.csv', row.names = FALSE)
...
setwd('C:/R Projects/Similkameen-WQOs/data/report/SC_Plots/High_ND')
...
#parameters <- distinct (SC_sites, Variable, Units)
...
for (v in ugL) {
  ugL_plots <- filter(SC_sites, Variable == v) %>%
    group_by(EMS_ID)
...
GM_sites %<>% filter(!( Variable == "aluminum dissolved" & CENSOR == TRUE & Value >= 0.2|
                         ...
                         Variable == "cyanide dissolved" & CENSOR == TRUE & Value >= 100|
                         Variable == "lead dissolved" & CENSOR == TRUE & Value >= 10| 
                         Variable == "molybdenum dissolved" & CENSOR == TRUE & Value >= 0.03|
                         Variable == "selenium dissolved" & CENSOR == TRUE & Value >= 1200|
                         Variable == "silver dissolved" & CENSOR == TRUE & Value >= 10|
                         Variable == "zinc dissolved" & CENSOR == TRUE & Value >= 15))
...
write.csv(SC_sites,'C:/R Projects/Similkameen-WQOs/data/report/SC_Plots/tables/SC_sites2.csv', row.names = FALSE)
