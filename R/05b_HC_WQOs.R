...
# This script will graph the parameters for Hedley Creek, plot WQGs/WQOs
...
# Load data that is ready for plotting with WQOs, this is SR_sites3
plotdata <- read_csv("data/report/HC_Plots/tables/HC_sites2.csv")

#no data from the Cu BLM or NH4 calculated guideline dataframes
...
plotdata$EMS_ID <- factor(plotdata$EMS_ID, levels=c("E223873", "E223874"))

#parm <- distinct(plotdata, Variable, Units)

# set working drive for plots
setwd('C:/R Projects/Similkameen-WQOs/data/report/HC_Plots/WQOs')
...
#remove results <MDL 3.0
site_tss <- site_tss %>% dplyr::filter(!(Variable == "tss" & Value == 3))
...
# load TSS function and run (manually)
#run TSS WQO
(tsslimits<-TSS_WQO(site_tss, "E223873", 10, 10, 100))

#write csv of TSS threshold, data shows clear flows fo threshold is 10 mg/L 
write.csv(tsslimits,'C:/R Projects/Similkameen-WQOs/data/report/HC_Plots/tables/HC_tss.csv', row.names = FALSE)

## plot tss against threshold for background
...
## plot tss against threshold for E223873
...
tssplot2 <- ggplot(tsslimits2, aes(x = DateTime, y = Value, col = Variable, shape=CENSOR)) +
  ...
  labs(col = "E223873", shape="<MDL")
plot(tssplot2)
ggsave(filename = "TSS_WQO E223873.png", plot = tssplot2, h=4, w=6, units="in", dpi=300)

#Dissolved Calcium
site_ca <- filter(plotdata, Variable %in% "calcium dissolved")
#only 2 data points, one for each station in early 2000, filtering out data >2015
site_ca <- filter(site_ca, DateTime > "2015-01-01" & DateTime < "2019-12-31")
#plot
...
#only 2 data points, one for each station in early 2000, filtering out data >2012
site_mg <- filter(site_mg, DateTime > "2012-01-01" & DateTime < "2019-12-31")
#plot
...
#only 2 data points, one for each station in early 2000, filtering out data >2012
site_hard <- filter(site_hard, DateTime > "2015-01-01" & DateTime < "2019-12-31")
...
crplot <- ggplot(site_cr, aes(x = DateTime, y = Value, color = EMS_ID, shape = CENSOR)) +
  geom_point(size=0.75) + 
  geom_hline(aes(yintercept = 20,linetype = "Acute"), color = "blue") +
  geom_hline(aes(yintercept = 2,linetype = "Chronic"), color = "red") +
  scale_linetype_manual(name = "T Cr WWQO", values = c(2, 2),  
...
crplot <- ggplot(site_cr, aes(x = DateTime, y = Value, color = EMS_ID, shape = CENSOR)) +
  ...
  geom_hline(aes(yintercept = 1,linetype = "WWQG Chronic (Cr(VI)"), color = "red") +
  ...
  theme(text=element_text(size=8)) +
  xlab("Date") +
  ylab("Chromium"~~(mu*g/L)) + coord_cartesian(ylim=c(0,2)) +
  theme(text=element_text(size=8)) +
  labs(color="Station", shape="<MDL")+
  facet_wrap(~Variable, ncol=2, labeller = as_labeller(plotnames))
plot(crplot)
...
wadplot <- ggplot(site_wad, aes(x = DateTime, y = Value, color = EMS_ID, shape = CENSOR)) +
  geom_point(size=0.75) + 
  geom_hline(aes(yintercept = 10,linetype = "Acute"), color = "blue") +
...
#remove results <MDL 0.05
plotdata <- plotdata %>% 
  dplyr::filter(!((Variable == "arsenic total" |Variable == "arsenic dissolved") & Value==0.5))
...
asplot <- ggplot(site_as, aes(x = DateTime, y = Value)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Total Arsenic"~~(mu*g/L)) + ylim(0,10) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
...
asplot <- ggplot(site_as, aes(x = DateTime, y = Value, shape = CENSOR)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Dissolved Arsenic"~~(mu*g/L)) + ylim(0,10) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  labs(shape="<MDL")+
...
hgplot <- ggplot(site_hg, aes(x = DateTime, y = Value, shape = CENSOR)) +
  geom_point(size=0.75) + 
  geom_hline(aes(yintercept = 1,linetype = "Chronic AL"), color = "blue") +
  geom_hline(aes(yintercept = 0.02,linetype = "Acute AL"), color = "red") +
  scale_linetype_manual(name = "WQO/WQG", values = c(2, 2),  
                        guide = guide_legend(override.aes = list(color = c("blue", "red")))) +
  theme_bw() +
  xlab("Date") +
...
urplot <- ggplot(site_ur, aes(x = DateTime, y = Value)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Total Uranium"~~(mu*g/L)) + coord_cartesian(ylim=c(0,1.5)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  labs(shape="<MDL")+
...
seplot <- ggplot(site_se, aes(x = DateTime, y = Value, shape = CENSOR)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Total Selenium"~~(mu*g/L)) + coord_cartesian(ylim=c(0,3)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  labs(shape="<MDL")+
...
seplot <- ggplot(site_se, aes(x = DateTime, y = Value, shape = CENSOR)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Dissolved Selenium"~~(mu*g/L)) + coord_cartesian(ylim=c(0,3)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  labs(shape="<MDL")+
...
agplot <- ggplot(site_ag, aes(x = DateTime, y = Value, color = EMS_ID, shape = CENSOR)) +
  ...
  theme(text=element_text(size=8)) +
  xlab("Date") +
  ylab("Silver"~~(mu*g/L)) + coord_cartesian(ylim=c(0,0.15)) +
  theme(text=element_text(size=8)) +
  labs(color="Station", shape = "<MDL")+
  facet_wrap(~Variable, ncol=2, labeller = as_labeller(plotnames))
plot(agplot)
...
agplot <- ggplot(site_ag, aes(x = DateTime, y = Value, shape = CENSOR)) +
  ...
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) + coord_cartesian(ylim=c(0,0.15)) +
  labs(shape="<MDL")+
  theme(strip.text = element_text(size=4, margin = margin(.2, 0, .2, 0, "mm"))) +
  facet_wrap(~EMS_ID, ncol=2, drop=FALSE)
plot(agplot)
...
#few data points, filtering out data >2010
site_mn <- filter(site_mn, DateTime > "2010-01-01" & DateTime < "2019-12-31")
...
site_mn2 <- filter(site_mn, Variable %in% "manganese total")
...
site_mn3 <- filter(site_mn, Variable %in% "manganese dissolved")
mnplot <- ggplot(site_mn, aes(x = DateTime, y = Value)) +
  geom_point(size=0.75) + 
  theme_bw() +
  xlab("Date") +
...
site_mn <- bind_rows(WQGs, site_mn)

#T&D Mn E223873 w/WQGs

site_mn4 <- filter(site_mn, Variable %in% c("manganese total", 
                                            "manganese dissolved",
                                            "AcuteAL", "ChronicAL") 
                   & EMS_ID == "E223873")
#few data points, filtering out data >2010
site_mn4 <- filter(site_mn4, DateTime > "2010-01-01" & DateTime < "2019-12-31")
mnplot2 <- ggplot(site_mn4, aes(x = DateTime, y = Value, col = Variable)) +
  ...
  labs(col = "E223873")
plot(mnplot2)
ggsave(filename = "T+D MnE223873.png", plot = mnplot2, h=4, w=6, units="in", dpi=300)

#T&D Mn E223874 w/WQGs
site_mn5 <- filter(site_mn, Variable %in% c("manganese total", 
                                            "manganese dissolved",
                                            "AcuteAL", "ChronicAL") 
                   & EMS_ID == "E223874")
#few data points, filtering out data >2010
site_mn5 <- filter(site_mn5, DateTime > "2010-01-01" & DateTime < "2019-12-31")
mnplot2 <- ggplot(site_mn5, aes(x = DateTime, y = Value, col = Variable)) +
  ...
  labs(col = "E223874")
plot(mnplot2)
ggsave(filename = "T+D MnE223874.png", plot = mnplot2, h=4, w=6, units="in", dpi=300)
...
pbplot <- ggplot(site_pb, aes(x = DateTime, y = Value, color = EMS_ID, shape=CENSOR)) +
  ...
  theme(text=element_text(size=8)) +
  xlab("Date") +
  ylab("Lead"~~(mu*g/L)) + coord_cartesian(ylim=c(0,2.25)) +
  theme(text=element_text(size=8)) +
  labs(color="Station", shape="<MDL")+
  facet_wrap(~Variable, ncol=2, labeller = as_labeller(plotnames))
plot(pbplot)
...
pbplot <- ggplot(site_pb, aes(x = DateTime, y = Value, shape = CENSOR)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Total Lead"~~(mu*g/L)) + coord_cartesian(ylim=c(0,2.25)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  theme(strip.text = element_text(size=4, margin = margin(.2, 0, .2, 0, "mm"))) +
...
pbplot <- ggplot(site_pb, aes(x = DateTime, y = Value, shape = CENSOR)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Dissolved Lead"~~(mu*g/L)) + coord_cartesian(ylim=c(0,2.25)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  theme(strip.text = element_text(size=4, margin = margin(.2, 0, .2, 0, "mm"))) +
...
site_pb2 <- bind_rows(WQGs, site_pb2)

#T&D Pb E223873 w/WQGs
...
pbplot2 <- ggplot(site_pb3, aes(x = DateTime, y = Value, col = Variable, shape=CENSOR)) +
  ...
  theme(text=element_text(size=8)) +
  xlab("Date") +
  ylab("Concentration"~~(mu*g/L)) + coord_cartesian(ylim=c(0,25)) +
  labs(col = "E223873", shape= "<MDL")
plot(pbplot2)
ggsave(filename = "T+D pbE223873.png", plot = pbplot2, h=4, w=6, units="in", dpi=300)

#T&D Pb E223874 w/WQGs
...
pbplot2 <- ggplot(site_pb4, aes(x = DateTime, y = Value, col = Variable, shape=CENSOR)) +
  ...
  xlab("Date") +
  ylab("Concentration"~~(mu*g/L)) + 
  labs(col = "E223874", shape="<MDL)")
plot(pbplot2)
ggsave(filename = "T+D PbE223874.png", plot = pbplot2, h=4, w=6, units="in", dpi=300)
...
#remove results <MDL 0.05
site_cd <- site_cd %>% dplyr::filter(!(Variable == "cadmium total" & Value >= 0.05 |
                                         Variable == "cadmium dissolved" & Value >= 0.05))
...
site_cd <- site_cd %>% dplyr::filter(!(Variable == "cadmium dissolved" & Value >= 0.05))
cdplot <- ggplot(site_cd, aes(x = DateTime, y = Value, shape = CENSOR)) +
  geom_point(size=0.75) + 
  theme_bw() +
  xlab("Date") +
...
WQGs <- bind_rows(AcuteAL, ChronicAL, id=NULL)
#make df of T&D Cd
site_cd2 <- filter(plotdata, Variable %in% c("cadmium total", "cadmium dissolved"))
#merge WQGs with site_cd
site_cd2 <- bind_rows(WQGs, site_cd2)


#T&D Cd E223873 w/WQGs

site_cd3 <- filter(site_cd2, Variable %in% c("AcuteAL","ChronicAL",
                                            "cadmium total", 
                                            "cadmium dissolved")
                   & EMS_ID == "E223873")           
...
site_cd3$Variable <- factor(site_cd3$Variable, 
                            levels=c("AcuteAL", "ChronicAL", "cadmium total", 
                                     "cadmium dissolved"))


cdplot2 <- ggplot(site_cd3, aes(x = DateTime, y = Value, col = Variable, shape=CENSOR)) +
  geom_point(size=0.75) + 
  theme_bw() +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  xlab("Date") +
  ylab("Concentration"~~(mu*g/L)) + coord_cartesian(ylim=c(0,0.5))+
  labs(col = "E223873", shape="<MDL")
plot(cdplot2)
ggsave(filename = "T+D cdE223873.png", plot = cdplot2, h=4, w=6, units="in", dpi=300)

#T&D Cd E223874 w/WQGs
site_cd4 <- filter(site_cd2, Variable %in% c("cadmium total", 
                                            "cadmium dissolved",
                                            "AcuteAL", "ChronicAL") 
                   & EMS_ID == "E223874")
#plot data in specific order
site_cd4$Variable <- factor(site_cd4$Variable, 
                            levels=c("AcuteAL", "ChronicAL", "cadmium total", 
                                     "cadmium dissolved"))
cdplot2 <- ggplot(site_cd4, aes(x = DateTime, y = Value, col = Variable, shape=CENSOR)) +
  geom_point(size=0.75) + 
  theme_bw() +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  ...
  labs(col = "E223874", shape="<MDL")
plot(cdplot2)
ggsave(filename = "T+D CdE223874.png", plot = cdplot2, h=4, w=6, units="in", dpi=300)
...
niplot <- ggplot(site_ni, aes(x = DateTime, y = Value, shape = CENSOR)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Total Nickel"~~(mu*g/L)) + coord_cartesian(ylim=c(0,1)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  theme(strip.text = element_text(size=4, margin = margin(.2, 0, .2, 0, "mm"))) +
...
znplot <- ggplot(site_zn, aes(x = DateTime, y = Value, color = EMS_ID, shape=CENSOR)) +
  ...
  theme(text=element_text(size=8)) +
  xlab("Date") +
  ylab("Zinc"~~(mu*g/L)) + coord_cartesian(ylim=c(0,35)) +
  theme(text=element_text(size=8)) +
  labs(color="Station", shape="<MDL")+
  facet_wrap(~Variable, ncol=2, labeller = as_labeller(plotnames))
plot(znplot)
...
znplot <- ggplot(site_zn, aes(x = DateTime, y = Value, shape = CENSOR)) +
  geom_point(size=0.75) + 
  geom_hline(aes(yintercept = 10,linetype = "Chronic AL"), color = "red") +
  scale_linetype_manual(name = "WQO T-Zn", values = c(1, 1),  
                        guide = guide_legend(override.aes = list(color = "red"))) +
  theme_bw() +
  xlab("Date") +
...
znplot <- ggplot(site_zn, aes(x = DateTime, y = Value, shape = CENSOR)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Dissolved Zinc"~~(mu*g/L)) + coord_cartesian(ylim = c(0,20)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  theme(strip.text = element_text(size=4, margin = margin(.2, 0, .2, 0, "mm"))) +
...
noplot <- ggplot(site_no, aes(x = DateTime, y = Value, color = EMS_ID, shape=CENSOR)) +
  ...
  scale_linetype_manual(name = "WQG T-NO2-", values = c(2, 2),  
                        guide = guide_legend(override.aes = list(color = c("blue", "red")))) +
  theme_bw() +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  xlab("Date") +
  ylab("Nitrite (mg/L)") + coord_cartesian(ylim=c(0,0.06)) +
  theme(text=element_text(size=8)) +
  labs(color="Station", shape="<MDL")+
  facet_wrap(~Variable, ncol=2, labeller = as_labeller(plotnames))
plot(noplot)
...
ntu_WQO <- turbidity_WQO(plotdata, "E223873", 1, 5, 10, 5, 50)

#write csv of TSS threshold, data shows clear flows for threshold is 10 mg/L 
write.csv(ntu_WQO,'C:/R Projects/Similkameen-WQOs/data/report/HC_Plots/tables/HC_turbidity.csv', row.names = FALSE)

## plot turbidity against threshold for background
...
## plot turbidity against threshold for E223873
...
ntuplot2 <- ggplot(ntu_WQO, aes(x = DateTime, y = Value, col = Variable)) +
  ...
  labs(col = "E223873", shape="<MDL")
plot(ntuplot2)
ggsave(filename = "Turbidity_WQO E223873.png", plot = ntuplot2, h=4, w=6, units="in", dpi=300)
...
ntuplot2 <- ggplot(ntu_WQO, aes(x = DateTime, y = Value, col = Variable)) +
  ...
  theme(text=element_text(size=8)) +
  xlab("Date") +
  ylab("Concentration"~~(mu*g/L)) + coord_cartesian(ylim=c(0,22)) +
  labs(col = "0500075", shape="<MDL")
plot(ntuplot2)
ggsave(filename = "Turbidity_WQO2 E223873.png", plot = ntuplot2, h=4, w=6, units="in", dpi=300)
...
Cuplot <- ggplot(site_Cu, aes(x = DateTime, y = Value, color = EMS_ID, shape=CENSOR)) +
  ...
  theme(text=element_text(size=8)) +
  xlab("Date") +
  ylab("Copper"~~(mu*g/L)) + coord_cartesian(ylim=c(0,15)) +
  theme(text=element_text(size=8)) +
  labs(color="Station", shape="<MDL")+
  facet_wrap(~Variable, ncol=2, labeller = as_labeller(plotnames))
plot(Cuplot)
...
Cuplot <- ggplot(site_Cu, aes(x = DateTime, y = Value, shape = CENSOR)) +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Total Copper"~~(mu*g/L)) + coord_cartesian(ylim = c(0,15)) +
  theme(legend.title = element_text(size = 6), legend.text = element_text(size = 6)) +
  theme(legend.key.size = unit(0.2, "cm")) + theme(legend.key = element_blank()) +
  theme(text=element_text(size=8)) +
  theme(strip.text = element_text(size=4, margin = margin(.2, 0, .2, 0, "mm"))) +
