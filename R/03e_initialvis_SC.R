...
setwd('C:/R Projects/SimilkameenWQOs/out/SunsetCreek/Recent/')

## make dataframe of Sunset Creek sites
SC_sites <- filter(sim_clean, EMS_ID == "E206634" | EMS_ID == "E250751" |
                     EMS_ID == "E215954")

## filter out the dates that I want to plot
SC_sites <- filter(SC_sites, DateTime > "2008-01-01" & DateTime < "2019-12-31")

## mg/L plots
site_mgL <- filter(SC_sites, Variable %in% mgL)
...
for (v in mgL) {
  ...
  mgL_plots <- filter(SC_sites, Variable == v) %>%
    group_by(EMS_ID)
...
site_NH4 <- filter(SC_sites, Variable == "nitrogen ammonia dissolved")
...
# Nitrate
site_NO3 <- filter(SC_sites, Variable == "nitrogen dissolved nitrate")
NO3plot <- ggplot(site_NO3, aes(x = DateTime, y = Value, color = EMS_ID)) +
  geom_point() + ylim(0, 20) +
  ggtitle("Dissolved Nitrate") +
  xlab("Date") +
  ylab("mg/L")
plot(NO3plot)
ggsave(filename = "nitrogen dissolved nitrate2.png", plot = NO3plot, units= "in")

## TSS
site_TSS <- filter(SC_sites, Variable == "tss")
...
SC_sites <-  SC_sites %>% mutate(Value=ifelse(Units == "mg/l", Value*1000, Value )) %>%
  mutate(Units = ifelse(Units == "mg/l", "ug/l", Units))

site_ugL <- filter(SC_sites, Variable %in% ugL)
...
## Dissolved As
site_As <- filter(SC_sites, Variable == "arsenic dissolved")
...
## Dissolved Cd
site_Cd <- filter(SC_sites, Variable == "cadmium dissolved")
...
## Dissolved Fe
site_Fe <- filter(SC_sites, Variable == "iron dissolved")
...
## Dissolved Pb
site_Pb <- filter(SC_sites, Variable == "lead dissolved")
...
## Dissolved Ni
site_Ni <- filter(SC_sites, Variable == "nickel dissolved")
...
## Dissolved Se
site_Se <- filter(SC_sites, Variable == "selenium dissolved")
...
## Dissolved Cr
site_Cr <- filter(SC_sites, Variable == "chromium dissolved")
...
## Dissolved Al
site_Al <- filter(SC_sites, Variable == "aluminum dissolved")
...
## Dissolved Mo
site_Mb <- filter(SC_sites, Variable == "molybdenum dissolved")
...
## CN- WAD
site_wad <- filter(SC_sites, Variable == "cyanide wad")
...
## CN- SAD
site_sad <- filter(SC_sites, Variable == "cyanide s.a.d.")
...
## Dissolved Ag
site_Ag <- filter(SC_sites, Variable == "silver dissolved")
...
site_pH <- filter(SC_sites, Variable == "ph")
...
site_temp <- filter(SC_sites, Variable == "temperature")
...
site_tur <- filter(SC_sites, Variable == "turbidity")
