...
# Load SR data
NP_sites <- read.csv("data/report/NPMC_plots/tables/NP_sites.csv", 
                     stringsAsFactors = FALSE, header=TRUE)

#remove all values <0 (negative values throw an error in ROS)
NP_sites <- NP_sites %>% filter(Value>0)

#remove unnescessary columns
NP_sites <- dplyr::select(NP_sites, -c(Code, ResultLetter, Station)) 

# Summary Stats for table 1 - based upon uncensored data 

table.1 <- NP_sites %>% group_by(EMS_ID, Variable) %>%
...
cens_data <- filter(NP_sites, CENSOR == TRUE)
...
write.csv(table.1,'data/report/NPMC_plots/tables/Table1_censored.csv', row.names = FALSE)

# Determine non-detects by station to select method.
# Count samples that are D vs ND
method_selection <- NP_sites %>%
...
# Join to data set
NP_sites %<>% left_join(select(method_selection, EMS_ID, Variable,
                               METHOD))
...
write.csv(method_summary,'data/report/NPMC_plots/tables/MethodSummary.csv', row.names = FALSE)
...
g1_data <- NP_sites %>% filter(METHOD == "0.5 MDL") %>%
  group_by(EMS_ID, Variable) %>%
...
# arithmetic mean
g2_data <- NP_sites %>% filter(METHOD == "0.5 MDL - MEAN") %>%
...
g3_data <- NP_sites %>% filter(METHOD == "ROS")
...
g4_data <- NP_sites %>% filter(METHOD == "MEAN") %>%
  group_by(EMS_ID, Variable) %>%
...
write.csv(summary_stats,'data/report/NPMC_plots/tables/SummaryStats.csv', row.names = FALSE)
...
sumstats <- read.csv("data/report/NPMC_plots/tables/SummaryStats.csv", 
                     stringsAsFactors = FALSE, header=TRUE)

## order sites from upstream to downstream
sumstats$EMS_ID <- factor(sumstats$EMS_ID, levels=c( "E217193", "E206633"))
...
for (i in 1:length(parameters)){
  ...
  ggsave(filename=paste0("C:/R Projects/Similkameen-WQOs/data/report/NPMC_Plots/",x,".tiff"), units="in", width=9, height=6, dpi=300, compression = 'lzw')
